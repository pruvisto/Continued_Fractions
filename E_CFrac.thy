section \<open>The continued fraction expansion of \<open>e\<close>\<close>
theory E_CFrac
imports
...
text \<open>
  The continued fraction expansion of \<open>e\<close> has the form
...
proof -
  ...
  have "((\<lambda>x. exp x * x) has_integral (exp 1 * (1 - 1) - exp 0 * (0 - 1))) {0..1::real}"
    by (intro fundamental_theorem_of_calculus)
       (auto intro!: derivative_eq_intros 
             simp: has_field_derivative_iff_has_vector_derivative [symmetric] algebra_simps)
...
proof -
  ...
  have "A (Suc n) + B n + C n =
    ...
    apply (subst integral_add [symmetric])
      apply (auto intro!: integrable_continuous_real continuous_intros)[2]
    apply (subst integral_add [symmetric])
...
lemma conv_diff_exp_bound:
  assumes "n \<noteq> 1"
  shows   "norm (exp 1 - p n / q n) \<le> exp 1 / fact (n div 3)"
proof -
  ...
  have "norm ((q n * exp 1 - p n) / q n) \<le> exp 1 / fact (n div 3) / 1" unfolding diff norm_divide 
    using A_bound[of "n div 3"] B_bound[of "n div 3"] C_bound[of "n div 3"] q_pos[OF assms]
    by (subst frac_le) (auto simp: of_nat_ge_1_iff)
  also have "(q n * exp 1 - p n) / q n = exp 1 - p n / q n"
    using q_pos[OF assms] by (simp add: divide_simps)
  finally show ?thesis by simp
qed
...
proof -
  ...
  proof (rule Lim_null_comparison)
    show "eventually (\<lambda>n. norm (exp 1 - p n / q n) \<le> exp 1 / fact (n div 3)) at_top"
      using eventually_gt_at_top[of "1::nat"] by eventually_elim (rule conv_diff_exp_bound, auto)
