...
next
  ...
  next
    ...
    proof (cases "cfrac_lim c \<in> \<int>")
      case True
      thus ?thesis
        using cfrac_int_cases[of c] by (auto simp: Ints_def)
    next
      case [simp]: False
      have "\<not>cfrac_is_int c"
...
lemma conv_best_approximation_ex:
  ...
  assumes "0 < b" and "b \<le> k n" and "coprime a b"
...
proof (cases "\<bar>a\<bar> = \<bar>h n\<bar> \<and> b = k n")
  ...
  next
    case False
    have "h n \<noteq> 0"
      using True assms(5) h_def by auto
...
next
  ...
  next
    ...
    hence "k n < k (Suc n)"
      by (auto simp: k_def intro: conv_denom_lessI)
    ...
    proof (cases "Suc n = cfrac_length c")
      ...
      proof
        ...
        have "b = (cfrac_nth c (Suc n) - 1) * k n + k (n - 1)"
          using eq2 \<open>n \<noteq> 0\<close> by (cases n) (auto simp: k_def algebra_simps)
...
lemma conv_best_approximation_ex_nat:
  ...
  using conv_best_approximation_ex[OF assms(1), of b a] assms by auto
...
proof -
  ...
  also have "\<dots> \<le> \<bar>b * x - a\<bar> / k n" unfolding x_def using assms *
    by (intro divide_right_mono conv_best_approximation_ex) auto
  ...
  proof (rule ccontr)
    ...
    next
      ...
      also have "\<bar>k r * x - h r\<bar> \<le> \<bar>b * x - a\<bar>"
        using assms r True unfolding x_def by (intro conv_best_approximation_ex) auto
